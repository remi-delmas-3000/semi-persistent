...
fn clustered() -> FrameStats {
    FrameStats { n: 500, runs: 1, distinct: 500 }
}
...
fn calibrate_then_promote_then_recalibrate() {
    ...
    for _ in 0..4 {
        assert!(is_auto(p.flush_mode()), "still calibrating during the window");
    ...
    assert!(is_runs(p.default), "clustered frames promote IndexRuns as default");
    assert!(is_runs(p.flush_mode()), "steady state flushes with the promoted default");

    // Phase 2 — default: flush uses the fixed default for `period` frames, no
    // per-frame decision paid.
    for _ in 0..8 {
        assert!(is_runs(p.flush_mode()), "default phase keeps the promoted mode");
        p.observe_frame(clustered(), T, I);
    }

    // Period elapsed -> re-enter calibration to re-check the default.
    assert!(p.calibrating, "period elapsed -> re-calibrating");
    assert!(is_auto(p.flush_mode()), "re-calibration flushes with Auto again");
}
