...
fn code_byte_width(distinct: usize) -> usize {
    if distinct <= 256 { 1 } else if distinct <= 65536 { 2 } else { 4 }
}
...
fn contiguous(n: usize, runs: usize) -> Vec<(u32, u32)> {
    ...
    let mut base = 0u32;
    for _ in 0..runs {
        for o in 0..per { out.push(((rng.next() & 0xFFFF_FFFF) as u32, base + o as u32)); }
        base += (per as u32) * 2; // gap so runs stay separate
    }
    let mut extra = base;
    while out.len() < n { out.push(((rng.next() as u32), extra)); extra += 2; }
    out
}
...
fn scattered_unique(n: usize) -> Vec<(u32, u32)> {
    ...
    while out.len() < n {
        let idx = (rng.next() % (n as u64 * 8)) as u32;
        if used.insert(idx) { out.push((v.wrapping_mul(2654435761), idx)); v += 1; }
    }
    out
}
...
fn distinct_values(d: &[(u32, u32)]) -> usize {
    d.iter().map(|e| e.0).collect::<std::collections::HashSet<_>>().len()
}
...
fn report(shape: &str, d: &[(u32, u32)]) {
    ...
    eprintln!("    idx-major wo     {idx_wo:>9}  {:.2}x  ({wo_runs} runs)", r(idx_wo));
