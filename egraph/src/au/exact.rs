...
/// One bare-pair memo entry for context subsumption: the optimal term of a
/// context-clean solve of `(l, r)` plus the support of its winning derivation.
struct CleanEntry<T, C> {
    term: T,
    support: Support<C>,
}

/// The session memo type the hybrid callers thread through: term ids in the
/// caller's pool, class ids of the snapshot, and the config's index word.
pub(crate) type SessionMemo<Cfg> = super::exact_memo::ExactMemo<
    <<Cfg as EGraphConfig>::Au as AuIds>::Term,
    ClassOf<Cfg>,
    <<Cfg as EGraphConfig>::Au as AuIds>::Index,
>;
...
/// is non-optimal under EVERY context by the projection bound), no transport
/// cell
...
/// every removed candidate (cycle-blocked above the incumbent, or
/// projection-pruned)
...
///
/// `pruning`: branch-and-bound on the projection lower bound;
...
fn solve_entry<Cfg: EGraphConfig, L: LitVal, const T: bool, const P: bool>(
    ...
    pruning: bool,
    subsumption: bool,
    session_memo: Option<&mut SessionMemo<Cfg>>,
...
///
/// `warm_incumbent`: a feasible term for the entry pair (for example an
/// MCGS incumbent), offered at the entry OR node before the solve so the
/// branch-and-bound pruning starts from its quality instead of the
/// generalize seed's.
/// Sound for any feasible term: an incumbent only tightens pruning.
#[allow(clippy::too_many_arguments)]
...
/// (`estimates::lb_pair`): a structural action is skipped when
...
/// parent stopped.
///
/// `subsumption` enables context-subsumption reuse; the reuse argument and
...
/// Projection-pruned candidates do NOT taint cleanliness: their bounds are built
...
/// the projection-bound argument. The one exception is a pruned AC
/// representation pair
...
fn solve_iterative<Cfg: EGraphConfig, L: LitVal, const T: bool, const P: bool>(
    ...
    pruning: bool,
    subsumption: bool,
    mut session_memo: Option<&mut SessionMemo<Cfg>>,
    node_budget: Option<u64>,
    ...
    Cfg::Policy: crate::config::StorePolicy<Cfg, T>,
{
    ...
    let mut budget_entries: u64 = 0;
    loop {
        // Node budget: a deterministic per-call bound
        // on solve work, counted in node entries. On exhaustion the loop
        // unwinds exactly like an expired deadline: the root frame's
        // incumbent comes back uncertified, and (with the session memo) every
        // subframe that completed stays earned. Deterministic where the
        // wall-clock deadline is not, so certification under the budget is
        // reproducible.
        if let Some(limit) = node_budget {
            budget_entries += 1;
            if budget_entries > limit && !stack.is_empty() {
        ...
        match memo[or_id.to_usize()] {
            ...
            MemoState::Empty => {
                ...
                if l == r {
                    ...
                    && let Some((term, support)) = {
                        // Bare-pair reuse, two tiers: the in-call memo first,
                        // then the session memo (same context-clean reuse rule; the
                        // entry merely outlives the call). Either hit needs
                        // its support disjoint from both entry contexts.
                        let ctx_l = space
                            .contexts
                        ...
                        if let Some(entry) = sub.by_pair.get(&(l.to_usize(), r.to_usize()))
                            && sorted_disjoint(&entry.support.0, ctx_l)
                            && sorted_disjoint(&entry.support.1, ctx_r)
                        {
                            Some((entry.term, entry.support.clone()))
                        } else if let Some(memo_ref) = session_memo.as_deref()
                            && let Some((term, sl, sr)) =
                                memo_ref.get(l.to_usize() as u64, r.to_usize() as u64)
                            && sorted_disjoint(sl, ctx_l)
                            && sorted_disjoint(sr, ctx_r)
                        {
                            Some((term, (sl.to_vec(), sr.to_vec())))
                        } else {
                            None
                        }
                    }
                {
                    // A clean solve of (l, r) exists and its support is
                    // disjoint from both entry contexts, so its term is this
                    // state's exact optimum too (argument on
                    ...
                    loop {
                        ...
                        if *pair_idx == 0 && child_terms.is_empty() {
                            ...
                            if blocked {
                                ...
                                // every context by the projection-bound argument)
                                ...
                                // on variant mass (the strict-comparison guardrail).
                            ...
                            // Branch-and-bound: skip the action before any
                            ...
                            // context by the projection-bound argument.
                            if pruning
                                && action_size_bound(snap, pool, &action.pairs, &[], 0)
                                    > u64::from(frame.best_quality.0)
                            {
                            ...
                            && action_size_bound(snap, pool, &action.pairs, child_terms, *pair_idx)
                                > u64::from(frame.best_quality.0)
                        {
                            // Partial-sum tightening: solved children at
                ...
                Stage::Transport {
                    ...
                    loop {
                        ...
                        if *pair_idx < pairs.len() {
                            ...
                            let rows = lm.len();
                            let cols = rm.len();
                            // Branch-and-bound on the whole representation
                            // pair: `transport_pair_lb` (shared with MCGS
                            // dominance pruning) is a lower bound on the
                            ...
                            if pruning {
                                ...
                                if prune {
                                    // Unlike structural projection prunes, this
                        ...
                        if subsumption && frame.clean {
                            ...
                            if let Some(memo_ref) = session_memo.as_deref_mut() {
                                // The clean solve outlives this call in the session memo.
                                // First writer wins, like the in-call memo.
                                memo_ref
                                    .insert_if_absent(
                                        frame.l.to_usize() as u64,
                                        frame.r.to_usize() as u64,
                                        frame.best,
                                        sl.clone(),
                                        sr.clone(),
                                    )
                                    .expect("session memo sized by its index word");
                            }
