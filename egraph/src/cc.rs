...
impl<Cfg: EGraphConfig> CcSnapshot<Cfg>
where
    MSetCanon: VarCanon<Cfg::G, Cfg::C>,
{
        ...
        Cfg::Policy: crate::config::StorePolicy<Cfg, TRACK>,
    {
        ...
        completion_nodes.sort_unstable();
        completion_nodes.dedup();
    ...
    pub fn completion_nodes(&self) -> &[Cfg::G] {
        &self.completion_nodes
    }
...
mod tests {
    ...
    /// Brute-force reference: AC nodes of `op` whose `mset_children` contain `cr`.
    fn ref_partners(
    ...
    fn snapshot_matches_by_contains_variadic_fixture() {
        ...
        // completion_nodes lists exactly the two AC sums (not the op0 leaves).
        assert_eq!(snap.completion_nodes(), &[pxy.min(pxz), pxy.max(pxz)]);
    }
    ...
    fn snapshot_only_indexes_ac_ops() {
        ...
        assert_eq!(snap.completion_nodes(), &[pfx]);
    ...
    #[test]
    fn cc_collapsed_leaves_completion_set_but_stays_matchable() {
        ...
        assert!(
            CcSnapshot::build(&eg).completion_nodes().is_empty(),
