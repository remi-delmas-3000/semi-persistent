...
| `runtime_apply_configured_rollover`, `runtime_rollover_on_mark`, `runtime_push_frame_fallback`, `push_frame` | open store-selected ingress, then migrate only closed prefixes | shrink-capacity, Defer/configured/forced-edge, empty-frame, all-tier restore and SyncGroup suites | checked Hot-Defer marking and explicit-option dispatch are retained; configured/forced/all-tier fallback and legacy `push_frame` wrapper remain trusted |
