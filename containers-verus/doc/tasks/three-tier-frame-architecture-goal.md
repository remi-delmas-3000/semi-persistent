...
| `runtime_apply_configured_rollover`, `runtime_rollover_on_mark`, `runtime_push_frame_fallback` | open store-selected ingress, then migrate only closed prefixes | shrink-capacity, Defer/configured/forced-edge, empty-frame, all-tier restore and SyncGroup suites | checked general opening and explicit-Defer marking cover mixed histories; Hot specialization and public dispatch contracts are retained; configured/forced fallback remains trusted pending concrete rollover preservation |
