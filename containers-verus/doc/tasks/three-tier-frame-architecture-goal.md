...
| `runtime_push_fallback`, `runtime_pop_fallback` | immutable DiffStore capability selects Trail or Hot ingress | duplicate, pop/re-entry, three-protocol policy-matrix, and compatibility tests | all-tier capture and set, all-Hot specializations and public/runtime dispatch wrappers are checked; pop/push fallback bodies still require discharge |
