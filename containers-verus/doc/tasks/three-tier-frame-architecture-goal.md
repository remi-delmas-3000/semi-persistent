...
| `runtime_apply_configured_rollover`, `runtime_rollover_on_mark`, `runtime_push_frame_fallback` | open store-selected ingress, then migrate only closed prefixes | shrink-capacity, Defer/configured/forced-edge, empty-frame, all-tier restore and SyncGroup suites | checked Hot-Defer marking and explicit-option dispatch are retained; legacy `push_frame` wrapper now checks against the dispatcher contract; configured/forced/all-tier fallback remains trusted |
