...
| (this commit) | Extended goal 2: store policy for every composite (`store_policy::{TaggedFamily, PlainFamily, HotFirst, TrailFirst}`, `P = HotFirst` on `UnionFind`, `SparseSet`, `CircularList`, `ListArena`, `BPlusTreeSet`, `EClasses`), `DiffStore::lemma_wf_data_len` |
