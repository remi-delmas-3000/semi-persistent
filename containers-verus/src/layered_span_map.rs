...
//! it every round pays to rebuild the base value pool even when the round
//! changed a handful of keys. `LayeredSpanMap<V>` keeps the previous round's map as a `base`,
...
///
/// The pairwise phrasing (`forall|i, j| i < j ==> s[i] < s[j]`) creates
/// quadratic trigger instantiations, so the pairwise fact is derived on demand
/// by [`lemma_ascending_pairwise`].
...
impl<V: Copy + Default> LayeredSpanMap<V> {
    ...
    /// Structural well-formedness. The refinement is the separate `refines()`.
...
impl<V: Copy + Default> LayeredSpanMap<V> {
    ...
    /// binary search. The base value pool is not read. The current dense delta
    /// builder costs O(delta_stream + invalid + num_keys), because its fresh
    /// arena grows a span-table slot for every key.
    ...
    /// untouched. The base value pool is not read, but the current dense delta
    /// builder still costs O(delta_stream + invalid + num_keys): it uses a fresh
    /// span arena and grows the table to `num_keys`.
    /// `doc/design/16-layered-span-map.md` section 4 states the
    /// accumulate-and-reinstall policy this implements.
    ///
    ...
    /// logical contents. O(base + delta + key space): the output stream scans
    /// every key and the fresh result arena grows to the same key space. The
    /// caller amortizes it against `needs_flatten`.
    ///
