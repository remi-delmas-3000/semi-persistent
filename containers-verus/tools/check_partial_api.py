...
1. Every public exec fn whose signature carries a `requires` clause must be
   ...
   whatever the body does on violated preconditions. The compatibility
...
def scan_file(path: Path):
    ...
    for m in FN_RE.finditer(text):
        ...
        if has_req:
            # Invariant-only requires are this checker's class (a): the type's own
    ...
    if update:
        ...
        if allow_path.exists():
            header = ''.join(l for l in allow_path.read_text().splitlines(keepends=True)
                             if l.startswith('#'))
        if not header:
            header = ('# Public exec functions that still expose a `requires` clause.\n'
